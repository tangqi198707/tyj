...
*/	
	...
	//alert(codeHtml);
	var obj = msg ? msg : $('#msgContent');
	obj.val(codeHtml);
}
var rangeSel,range;
$(function(){
	create();
	$(".eItem").each(function(i){
		$(this).hover(
			function(){
				$(this).parent().css('borderColor','blue');
				$('.emotionsGif').html('<img src="/media/images/emotions/'+i+'.gif"/>');
			},
			function(){
		...
		$(this).click(function(){
			...
			insertHTML('<img src="/media/images/emotions/'+i+'.gif"/>');
	...
	$('.emotionsWord').click(function(){
		$('.emotions').toggle('fast');
	});
...
{ 
	var dthis=$("#editDiv")[0];//要插入内容的某个div,在标准浏览器中 无需这句话 
    if (window.getSelection)//ff
    {
        // IE9 and non-IE 
        ...
        if (range && (range.startContainer == 'HTMLDivElement' || range.startContainer.parentNode.id == 'editDiv' || range.startContainer.id == 'editDiv' || range.startContainer.parentNode.offsetParent.id == 'editDiv') && rangeSel.getRangeAt && rangeSel.rangeCount) { 
        	...
        	$("#editDiv").html($("#editDiv").html()+html);
    	}
    }  
